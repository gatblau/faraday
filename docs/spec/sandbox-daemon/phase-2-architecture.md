...
| — | `keychain` module (`SecretResolver` backends `FileSecretResolver`/`KeychainSecretResolver` + `KeychainStore`/`KeyringStore` + `resolver_kind`/`build_resolver`, ADR-040) | `keychain` | 3 | Config, `keyring` crate | Low |
...
| `PYS_SECRET_RESOLVER` | enum `file` \| `keychain` | `file` | No | keychain (`resolver_kind`) | Selects the `SecretResolver` backend for `*_REF` config secrets and `api_key` `secretRef`s: `file` (`FileSecretResolver`) or `keychain` (`KeychainSecretResolver`, OS secure store) — ADR-040. Unknown value → `CFG_INVALID`. |
| `PYS_KEYCHAIN_SERVICE` | string | `faradayd` | No | keychain / credential | OS-keychain *service* name under which `KeychainSecretResolver` (C1) reads and `CredentialCli` (C18) writes an `api_key` key; the `secretRef` is the keychain *account* (ADR-040). Used only when `PYS_SECRET_RESOLVER=keychain`. |
