...
## Amendment re-audit — revision 0.4.2 (`/spec` Branch B, 2026-07-17)

ADR-040 (Accepted) realises the keychain-backed `api_key` credential on the workstation: a **`KeychainSecretResolver`** (C1) behind the existing `SecretResolver` SPI, selected by **`PYS_SECRET_RESOLVER`** (`file`|`keychain`) with **`PYS_KEYCHAIN_SERVICE`**, and a new **C18 CredentialCli** (`faradayd credential set|rm|list`) for self-service per-user enrolment (resolves OQ-KC-1). The `api_key` auth mode / `secretRef` / `keyPlacement` (ADR-036) are unchanged — only the key's origin. **LLD changes:** **C1 Config** (KeychainSecretResolver + `PYS_SECRET_RESOLVER`/`PYS_KEYCHAIN_SERVICE` selection + per-resolver `secretRef` interpretation); **new C18 CredentialCli** (full spec); **Phase 2B** inventory + **Phase 2D** two env vars; **Phase 1** adds **AS-25**; **Phase 5** adds **Step 9**. Every new assertion is tagged **[Not implemented]** (no code yet — the ADR-040 remediation set). All component pins re-pinned `0.4.1 -> 0.4.2`.

**Re-audit verdict: PASSED.** Checklist re-verified for the delta:
- **C1 amendment** retains its full interface, Internal Logic (now +resolver selection), Error Table, Gherkin, `Gaps: None`; the new resolver + selection are marked [Not implemented].
- **C18 complete** — File, Dependencies, Purpose, Public Interface (`set|rm|list` + `run_credential`), 6-step numbered Internal Logic, a 5-row Error Table (`CRED_USAGE`/`CRED_UNKNOWN_REF`/`CRED_EMPTY`/`CRED_NOT_FOUND`/`CRED_KEYCHAIN`), 5 Gherkin scenarios (happy/edge/error), `Gaps: None blocking`.
- **Security** — C18 is a **local CLI run by the workstation user** (not a network entry point); it writes the secret only to the OS store, never echoes/logs it, never accepts it as an argv value; `set` validates the ref against the admin-signed manifest (C4/ADR-021). The key still never reaches the guest (ADR-013/019) and never appears in a response or audit entry (ADR-002). No new daemon attack surface.
- **Build order** — C18 depends on Config + PolicyEngine + the OS-keychain crate (already in Step 0); a CLI subcommand of the same binary, added at Step 9. **DAG still acyclic.**
- **No banned phrases; active locale en-GB; >=2 error rows; >=3 Gherkin per new/touched component; every new env var carries type/default/required/owner (Phase 2D).**
- **No invented identifiers presented as existing** — `SecretResolver`/`FileSecretResolver`/`ApiKeyStore`/`secretRef`/`keyPlacement` are verified in code; `KeychainSecretResolver`/`CredentialCli`/`PYS_SECRET_RESOLVER`/`PYS_KEYCHAIN_SERVICE`/`run_credential`/`KeychainStore` are **new [Not implemented]** identifiers introduced by ADR-040, traceable to the ADR, not claimed to exist in source.
- **[Not implemented] discipline** — every ADR-040 assertion is tagged; none is presented as existing code; they form the Step 9 remediation set for `codegen`.
- **D-HLD checks:** ADR-040 lives in HLD `09-decisions` (D-HLD-2 clean); new component **C18** appears in ADR-040 + Phase 2B/Phase 3 (D-HLD-4 clean); pins current at **0.4.2** (D-HLD-3 clean); changelog line present for the 0.4.2 bump (D-HLD-6 clean); **D-HLD-5:** the `11-policy-schema.md` `secretRef` note updated to the resolver-dependent interpretation (file path | keychain account), so siblings agree; **D-HLD-1:** no new HLD glossary term required. **The spec set is Approved at revision 0.4.2; ready for `/breakdown`.**

## Post-implementation note — ADR-040 realised (2026-07-17, `/apply` + `/sync-check` + `/sync-spec`)

ADR-040 was implemented (`sandbox-daemon` commit `8dc24ed`, three `/apply` phases) and the specs reconciled to code. **The `[Not implemented]` tags recorded in the 0.4.2 amendment above are now cleared** — C1 `KeychainSecretResolver` (`src/keychain.rs`), the `PYS_SECRET_RESOLVER`/`PYS_KEYCHAIN_SERVICE` selection (`keychain::resolver_kind`/`build_resolver`), `broker::freeze_api_keys`, and C18 `CredentialCli` (`src/credential.rs` + `main.rs`) all exist and are tested (71 lib tests; clippy/fmt/secscan clean). **Sync-spec refinements (code → spec):** C18's interface is the shipped `parse_cmd` + `run_credential(cmd, service, policy: &PolicyEngine, keychain) -> Vec<String>` split (not the single `run_credential(args, cfg, policy: &Manifest) -> ()` first drafted); the `keychain` module is inventoried (Phase 2B); the two ADR-040 env vars' owner is `keychain`/`credential`, not `Config`. The daemon-boot end-to-end smoke is deferred as **FU-004** (the seam is covered by lib integration tests). Revision remains **0.4.2** — implementation of an already-specced increment, no new intent. **Spec set Approved and code-aligned.**
