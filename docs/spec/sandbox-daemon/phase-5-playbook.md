...
## Step 9 — Keychain-backed api_key credential (ADR-040, remediation set — all [Not implemented])
- [ ] **C1 KeychainSecretResolver** — a second `SecretResolver` impl reading `(service=PYS_KEYCHAIN_SERVICE, account=secretRef)` from the OS secure store via the OS-keychain crate (Step 0); `PYS_SECRET_RESOLVER` (`file`|`keychain`) selects the impl at startup and injects it into `load` + the C11 `ApiKeyStore` freeze. Tests: keychain-hit resolves, keychain-miss → `CFG_SECRET_UNRESOLVED`, file-mode unchanged, unknown-selector → `CFG_INVALID`. Use a stub/in-memory keychain.
- [ ] **C18 CredentialCli** — `faradayd credential set|rm|list <secretRef>` dispatched from `main.rs` (alongside `mcp-stdio`/`install-mcp-config`); stdin no-echo, never an argv value; `set` validates the ref against the admin-signed manifest (C4); writes via the same keychain integration C1 reads. Tests: set-stores-no-echo, set-unknown-ref→`CRED_UNKNOWN_REF`, set-empty→`CRED_EMPTY`, rm-deletes, list-names-only. Secret value never logged.
- [ ] **Integration:** enrol a key via `credential set`, start the daemon with `PYS_SECRET_RESOLVER=keychain`, assert an `api_key` capability resolves the enrolled key from the OS store and applies it at its `keyPlacement` — with no key in any response, log, or audit entry.
