...
- [ ] **Integration (FU-004):** enrol a key via `credential set`, start the daemon with `PYS_SECRET_RESOLVER=keychain`, assert an `api_key` capability resolves the enrolled key from the OS store and applies it at its `keyPlacement` — with no key in any response, log, or audit entry. (Daemon-boot e2e deferred; the seam is covered by the lib integration tests above.)
