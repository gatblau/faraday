...
- [ ] **C17 McpUpstreamClient (ADR-034)** — outbound MCP **client**: streamable-HTTP `initialize` + `tools/call`; HTTPS-only with the ADR-032 loopback exception; no-cross-origin-redirect; size-capped read; returns the raw tool result for C5. Tests: tools/call-returns-result, 3xx-not-followed, upstream-isError-surfaced, timeout, remote-host-not-downgraded. Mock MCP server over an httptest server. (Distinct from C16, the inbound front door.)

## Step 3 — Broker + runtime
- [ ] **C11 IdentityBroker** — capability table + `mint_caps`, route to OBO/direct (`call`) and MCP (`call_tool` → C17, ADR-034), sanitise (C5 `sanitize`/`sanitize_mcp`), audit; reject kind/entrypoint mismatch as `CAP_INVALID`; tokens never leave. Tests: exchange-proxied, direct-provider, expired-capId, mcp-routed-to-upstream, rest-capId-on-call_tool→CAP_INVALID. Wire C5/C9/C10/C17/C3.
- [ ] **C12 SandboxRuntime** — Wasmtime hardened config (ADR-019: deny-by-default WASI subset = clock/random/captured-stdio only), guest digest verify (ADR-018), single **capability** host import → broker shim carrying a tagged `Rest`/`McpTool` request (ADR-034, still one import), fuel/epoch/memory limits, RustPython guest + `pysandbox_sdk`. Tests: guest-calls-api, guest-calls-mcp-tool, deadline-terminates, tampered-artefact-fails-closed.
...
- [ ] **Integration:** end-to-end over a real UDS — a test client connects (token + peer-UID), runs code that calls a capability against a mock obo-broker + mock downstream; assert: happy path, dry-run (no egress), unauthenticated-refused, traversal-denied, step-up challenge→consent-UI→retry, downstream-timeout. **MCP (ADR-034):** run code that calls `mcp.<cap>.call_tool` against a mock downstream MCP server; assert the allowed tool returns an `UntrustedMcpResult` with no token, a tool not in `toolAllow` is denied (`MCP_TOOL_DENIED`), and a returned resource link is not auto-dereferenced.
...
## Step 9 — Keychain-backed api_key credential (ADR-040) — implemented
- [x] **C1 KeychainSecretResolver** (`src/keychain.rs`) — a second `SecretResolver` impl reading `(service=PYS_KEYCHAIN_SERVICE, account=secretRef)` from the OS secure store via the `keyring` crate; `PYS_SECRET_RESOLVER` (`file`|`keychain`) selects the impl at startup (`resolver_kind`/`build_resolver`) and injects it into `Config::load` + `broker::freeze_api_keys`. Tests: keychain-hit resolves, keychain-miss → `CFG_SECRET_UNRESOLVED`, file-mode unchanged, unknown-selector → `CFG_INVALID` (in-memory `KeychainStore` fake).
- [x] **C18 CredentialCli** (`src/credential.rs` + `src/main.rs`) — `faradayd credential set|rm|list <secretRef>` dispatched from `main.rs` (alongside `mcp-stdio`/`install-mcp-config`); stdin no-echo, never an argv value; `set` validates the ref against the admin-signed manifest (C4); writes via the same keychain integration C1 reads. Tests: set-stores-no-echo, set-unknown-ref→`CRED_UNKNOWN_REF`, set-empty→`CRED_EMPTY`, rm-deletes, list-names-only. Secret value never logged.
- [x] **Integration (FU-004):** the enrol → resolve → apply round-trip is covered end-to-end by `tests/keychain_e2e.rs` (`--features integration`): `run_credential(Set…)` writes to a keychain, `freeze_api_keys` + `KeychainSecretResolver` reads it back, and the real `IdentityBroker` applies it at the capability's `keyPlacement` against a `mockserver` container — with no key in the response or audit. (The real-OS-keychain + real-process-boot variant remains FU-001.)
